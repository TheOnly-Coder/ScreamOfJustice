...
export const GameCanvas: React.FC<GameCanvasProps> = ({
  ...
  const c4DialogueTextRef = useRef<HTMLDivElement>(null);
  const c4DialogueSpeakerRef = useRef<HTMLDivElement>(null);
  const c4FadeRef = useRef<HTMLDivElement>(null);
  const c4Choice1Ref = useRef<HTMLButtonElement>(null);
  const c4Choice2Ref = useRef<HTMLButtonElement>(null);
  ...
  const gameRef = useRef<{
    ...
    // Campaign 4 state
    c4Phase: string;
    c4CutsceneTime: number;
    c4DialogueSpeaker: string;
    c4DialogueText: string;
    c4DialogueCharIdx: number;
    c4DialogueLastCharTime: number;
    c4FadeAlpha: number;
    c4FadeDir: number;
    c4TruckRef: THREE.Group | null;
    c4BriggsRef: THREE.Group | null;
    c4StrangerRef: THREE.Group | null;
    c4ChoiceShown: boolean;
    c4ChoiceMade: string | null;
    c4StrangerNearTruck: boolean;
    c4StrangerOnBoard: boolean;
    c4StrangerShot: boolean;
    c4BaseNPCs: THREE.Group[];
    c4WoundedNPCs: THREE.Group[];
    c4MedicNPCs: THREE.Group[];
    c4DialogueQueue: { speaker: string; text: string }[];
    c4InteractTarget: string | null;
    c4InteractCooldown: number;
    c4WaitingForEnd: boolean;
    c4EndTime: number;
    c4PlayerCanMove: boolean;
    c4ChoicePhase: string; // 'drive_or_scout', 'let_on_board'
    ...
    c4ChoiceShown: false,
    c4ChoiceMade: null,
    c4StrangerNearTruck: false,
    c4StrangerOnBoard: false,
    c4StrangerShot: false,
    c4BaseNPCs: [],
    c4WoundedNPCs: [],
    c4MedicNPCs: [],
    c4DialogueQueue: [],
    c4InteractTarget: null,
    c4InteractCooldown: 0,
    c4WaitingForEnd: false,
    c4EndTime: 0,
    c4PlayerCanMove: false,
    c4ChoicePhase: '',
    wantsToFire: false,
  ...
  useEffect(() => {
    ...
    if (config.isCampaign && config.mapId === 'tutorial') {
      ...
      game.weaponGroup = weaponGroup;
      weaponGroup.visible = false;
    } else if (config.isCampaign && config.mapId === 'campaign4') {
      // ===== CAMPAIGN 4: 'The Signal' - Branching Narrative =====
      game.hasWeapon = false;
      game.c4CutsceneTime = 0;
      game.c4Phase = 'truck_driving'; // truck_driving -> truck_stops -> fade_black -> fade_in_playable -> warning_message -> choice_drive_or_scout
      game.c4DialogueSpeaker = '';
      game.c4DialogueText = '';
      game.c4DialogueCharIdx = 0;
      game.c4DialogueLastCharTime = 0;
      game.c4FadeAlpha = 1; // Start fully black (fade in)
      game.c4FadeDir = -1;
      game.c4TruckRef = null;
      game.c4BriggsRef = null;
      game.c4StrangerRef = null;
      game.c4ChoiceShown = false;
      game.c4ChoiceMade = null;
      game.c4StrangerNearTruck = false;
      game.c4StrangerOnBoard = false;
      game.c4StrangerShot = false;
      game.c4BaseNPCs = [];
      game.c4WoundedNPCs = [];
      game.c4MedicNPCs = [];
      game.c4DialogueQueue = [];
      game.c4InteractTarget = null;
      game.c4InteractCooldown = 0;
      game.c4WaitingForEnd = false;
      game.c4EndTime = 0;
      game.c4PlayerCanMove = false;
      game.c4ChoicePhase = '';

      // Find truck in scene
      scene.traverse((child) => {
        if (child instanceof THREE.Group && !game.c4TruckRef) {
          child.traverse((c) => {
            if ((c as THREE.Mesh).geometry?.parameters?.width === 3 && (c as THREE.Mesh).geometry?.parameters?.height === 2.5) {
              game.c4TruckRef = child.parent instanceof THREE.Group ? child.parent : child;
            }
          });
        }
      });

      // Create Briggs model (olive drab soldier, stands near player)
      const briggsGroup = new THREE.Group();
      const bBody = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.2, 0.4), new THREE.MeshStandardMaterial({ color: 0x4a5a3a, flatShading: true }));
      bBody.position.y = 0.6;
      briggsGroup.add(bBody);
      const bHead = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.35), new THREE.MeshStandardMaterial({ color: 0xd4a574, flatShading: true }));
      bHead.position.y = 1.4;
      briggsGroup.add(bHead);
      const bHelmet = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.2, 0.4), new THREE.MeshStandardMaterial({ color: 0x3a4a2a, flatShading: true }));
      bHelmet.position.y = 1.6;
      briggsGroup.add(bHelmet);
      briggsGroup.position.set(-1.5, 0, -8);
      briggsGroup.visible = false; // Hidden during truck driving cutscene
      scene.add(briggsGroup);
      game.c4BriggsRef = briggsGroup;

      // Create stranger model (mysterious, dark clothing)
      const strangerGroup = new THREE.Group();
      const sBody = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.2, 0.4), new THREE.MeshStandardMaterial({ color: 0x2a2a2a, flatShading: true }));
      sBody.position.y = 0.6;
      strangerGroup.add(sBody);
      const sHead = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.35), new THREE.MeshStandardMaterial({ color: 0xc49464, flatShading: true }));
      sHead.position.y = 1.4;
      strangerGroup.add(sHead);
      const sHood = new THREE.Mesh(new THREE.BoxGeometry(0.42, 0.3, 0.42), new THREE.MeshStandardMaterial({ color: 0x222222, flatShading: true }));
      sHood.position.y = 1.55;
      strangerGroup.add(sHood);
      strangerGroup.position.set(20, 0, 3);
      strangerGroup.visible = false;
      scene.add(strangerGroup);
      game.c4StrangerRef = strangerGroup;

      // Create weapon group (hidden)
      const weaponGroup = new THREE.Group();
      scene.add(weaponGroup);
      game.weaponGroup = weaponGroup;
      weaponGroup.visible = false;
...
// 3. Rig Weapon Group to Camera (First Person Gun model)
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      // Campaign 4 choice handling
      if (config.isCampaign && config.mapId === 'campaign4' && game.c4ChoiceShown) {
        if (e.key.toLowerCase() === 'x') {
          if (game.c4ChoicePhase === 'drive_or_scout') {
            game.c4ChoiceMade = 'keep_driving';
            game.c4ChoiceShown = false;
    ...
    const animate = () => {
      ...
      // Skip for campaign3/campaign4 cutscene (camera is controlled by cutscene logic)
      if (!game.playerIsDead && !(config.isCampaign && config.mapId === 'campaign3') && !(config.isCampaign && config.mapId === 'campaign4' && !game.c4PlayerCanMove)) {
      ...
      // ===== CAMPAIGN 4: 'The Signal' - Branching Narrative =====
      if (config.isCampaign && config.mapId === 'campaign4') {
        const dt = delta;
        game.c4CutsceneTime += dt;
        const t = game.c4CutsceneTime;

        // --- Typewriter dialogue update ---
        if (game.c4DialogueCharIdx < game.c4DialogueText.length) {
          const now = performance.now();
          if (now - game.c4DialogueLastCharTime > 35) {
            game.c4DialogueCharIdx++;
            game.c4DialogueLastCharTime = now;
            if (c4DialogueTextRef.current) {
              c4DialogueTextRef.current.textContent = game.c4DialogueText.substring(0, game.c4DialogueCharIdx);
            }
            if (c4DialogueSpeakerRef.current) {
              c4DialogueSpeakerRef.current.textContent = game.c4DialogueSpeaker;
            }
          }
        }
        ...
        // --- Fade update ---
        if (game.c4FadeDir !== 0) {
          game.c4FadeAlpha += game.c4FadeDir * dt * 1.2;
          if (game.c4FadeAlpha <= 0) { game.c4FadeAlpha = 0; game.c4FadeDir = 0; }
          if (game.c4FadeAlpha >= 1) { game.c4FadeAlpha = 1; game.c4FadeDir = 0; }
        }
        if (c4FadeRef.current) {
          c4FadeRef.current.style.opacity = String(game.c4FadeAlpha);
        }

        // --- Phase state machine ---
        if (game.c4Phase === 'truck_driving') {
          // Phase 1: Truck driving (0-3s), then suddenly stops
          if (game.c4TruckRef) {
            if (t < 2.5) {
              game.c4TruckRef.position.z = -10 + t * 4;
            }
            // Truck stops abruptly at t=2.5
          }
          camera.position.set(0, 3.5, -10 + Math.min(t, 2.5) * 4 - 8);
          camera.lookAt(0, 2, -10 + Math.min(t, 2.5) * 4 + 5);
          if (t > 3.5) {
            game.c4Phase = 'fade_black';
            game.c4FadeDir = 1; // fade out
          }
        } else if (game.c4Phase === 'fade_black') {
          // Screen goes black, then reposition for playable scene
          if (game.c4FadeAlpha >= 1) {
            // Reposition truck to stop point
            if (game.c4TruckRef) {
              game.c4TruckRef.position.set(0, 0, 0);
            }
            // Position player next to truck
            game.playerPos.set(-2, 1.5, 0);
            game.yaw = 0;
            game.pitch = 0;
            // Show Briggs
            if (game.c4BriggsRef) {
              game.c4BriggsRef.visible = true;
              game.c4BriggsRef.position.set(-1.5, 0, 1);
            }
            camera.position.copy(game.playerPos);
            camera.rotation.set(0, 0, 0);
            game.c4Phase = 'fade_in_playable';
            game.c4FadeDir = -1; // fade in
            game.c4CutsceneTime = 0;
          }
        } else if (game.c4Phase === 'fade_in_playable') {
          if (game.c4FadeAlpha <= 0) {
            game.c4Phase = 'warning_message';
            game.c4CutsceneTime = 0;
            // Start warning message from '???'
            game.c4DialogueSpeaker = '???';
            game.c4DialogueText = 'Hostiles detected in the local area. You need to move. Now.';
            game.c4DialogueCharIdx = 0;
            game.c4DialogueLastCharTime = performance.now();
          }
        } else if (game.c4Phase === 'warning_message') {
          // Player stands next to truck, warning message plays, then Briggs speaks
          if (t > 4 && game.c4DialogueCharIdx >= game.c4DialogueText.length && game.c4DialogueQueue.length === 0) {
            // Briggs responds
            game.c4DialogueSpeaker = 'Sergeant Briggs';
            game.c4DialogueText = 'Could be a trap... or it could be real. Either way, we need to make a choice.';
            game.c4DialogueCharIdx = 0;
            game.c4DialogueLastCharTime = performance.now();
            game.c4Phase = 'briggs_choice_prompt';
            game.c4CutsceneTime = 0;
          }
        } else if (game.c4Phase === 'briggs_choice_prompt') {
          // Briggs offers the choice
          if (t > 4 && game.c4DialogueCharIdx >= game.c4DialogueText.length) {
            game.c4DialogueSpeaker = 'Sergeant Briggs';
            game.c4DialogueText = 'We can keep driving and take our chances on the road, or we can scout the area and take out anything we find. Your call, soldier. Press X to keep driving. Press V to scout the area.';
            game.c4DialogueCharIdx = 0;
            game.c4DialogueLastCharTime = performance.now();
            game.c4Phase = 'show_drive_or_scout_choice';
            game.c4ChoiceShown = true;
            game.c4ChoicePhase = 'drive_or_scout';
            game.c4CutsceneTime = 0;
            // Show choice buttons
            if (c4Choice1Ref.current) {
              c4Choice1Ref.current.style.display = 'flex';
            }
            if (c4Choice2Ref.current) {
              c4Choice2Ref.current.style.display = 'flex';
            }
          }
          ...
          if (game.c4TruckRef) {
            game.c4TruckRef.position.z = 0 + t * 5;
          }
          camera.position.set(0, 3.5, 0 + t * 5 - 8);
          camera.lookAt(0, 2, 0 + t * 5 + 5);
          if (t > 3) {
            // Truck approaches stranger standing on road
            if (game.c4StrangerRef) {
              game.c4StrangerRef.visible = true;
              game.c4StrangerRef.position.set(1.5, 0, 0 + t * 5 + 6);
            }
          }
          if (t > 5) {
            game.c4Phase = 'stranger_approach';
            game.c4CutsceneTime = 0;
          }
        } else if (game.c4Phase === 'stranger_approach') {
          // Stranger walks toward truck
          if (game.c4StrangerRef) {
            const strangerTarget = game.c4TruckRef ? game.c4TruckRef.position.z + 3 : 30;
            const currentZ = game.c4StrangerRef.position.z;
            if (Math.abs(currentZ - strangerTarget) > 1) {
              game.c4StrangerRef.position.z += dt * 2;
            }
          }
          if (t > 2) {
            // Stranger dialogue
            game.c4DialogueSpeaker = '?';
            game.c4DialogueText = 'Hey! Wait! I am on your team. Please, let me on. I know the area. I can help.';
            game.c4DialogueCharIdx = 0;
            game.c4DialogueLastCharTime = performance.now();
            game.c4Phase = 'stranger_let_choice';
            game.c4CutsceneTime = 0;
          }
        } else if (game.c4Phase === 'stranger_let_choice') {
          if (t > 5 && game.c4DialogueCharIdx >= game.c4DialogueText.length) {
            // Show choice
            game.c4ChoiceShown = true;
            game.c4ChoicePhase = 'let_on_board';
            if (c4Choice1Ref.current) {
              c4Choice1Ref.current.style.display = 'flex';
              c4Choice1Ref.current.querySelector('.choice-label')!.textContent = 'Let them on [X]';
            }
            if (c4Choice2Ref.current) {
              c4Choice2Ref.current.style.display = 'flex';
              c4Choice2Ref.current.querySelector('.choice-label')!.textContent = 'Keep going [V]';
            }
            game.c4Phase = 'waiting_for_stranger_choice';
            game.c4CutsceneTime = 0;
          }
          ...
          if (t > 4 && game.c4DialogueCharIdx >= game.c4DialogueText.length) {
            game.c4DialogueSpeaker = 'Sergeant Briggs';
            game.c4DialogueText = 'Alright, let us keep moving. Stay sharp, everyone.';
            game.c4DialogueCharIdx = 0;
            game.c4DialogueLastCharTime = performance.now();
            game.c4Phase = 'drive_off_with_stranger';
            game.c4CutsceneTime = 0;
          }
        } else if (game.c4Phase === 'drive_off_with_stranger') {
          // Truck drives away with stranger
          if (game.c4TruckRef) {
            game.c4TruckRef.position.z += dt * 6;
          }
          if (game.c4StrangerRef) {
            game.c4StrangerRef.position.z = game.c4TruckRef ? game.c4TruckRef.position.z + 1 : 30;
            game.c4StrangerRef.position.x = 0.5;
          }
          camera.position.set(0, 3.5, (game.c4TruckRef?.position.z || 0) - 8);
          camera.lookAt(0, 2, (game.c4TruckRef?.position.z || 0) + 5);
          if (t > 6) {
            game.c4Phase = 'ending_fade';
            game.c4FadeDir = 1;
            game.c4CutsceneTime = 0;
          }
        } else if (game.c4Phase === 'stranger_rejected_fade') {
          // PATH A-REJECT: Stranger rejected, fade to military base
          game.c4FadeDir = 1;
          if (game.c4FadeAlpha >= 1) {
            // Reposition to military base
            if (game.c4TruckRef) {
              game.c4TruckRef.position.set(0, 0, 70);
            }
            if (game.c4BriggsRef) {
              game.c4BriggsRef.position.set(-3, 0, 80);
              game.c4BriggsRef.visible = true;
            }
            game.playerPos.set(0, 1.5, 65);
            game.yaw = Math.PI / 2;
            camera.position.copy(game.playerPos);
            camera.rotation.set(0, game.yaw, 0);
            // Hide stranger
            if (game.c4StrangerRef) game.c4StrangerRef.visible = false;
            // Create base NPCs
            game.c4BaseNPCs = [];
            game.c4WoundedNPCs = [];
            game.c4MedicNPCs = [];
            const npcMat = new THREE.MeshStandardMaterial({ color: 0x4a5a3a, flatShading: true });
            const npcSkinMat = new THREE.MeshStandardMaterial({ color: 0xd4a574, flatShading: true });
            const medicMat = new THREE.MeshStandardMaterial({ color: 0xcc3333, flatShading: true });
            // Standing NPCs
            const standingPositions = [
              { x: -10, z: 78, name: 'Pvt. Adams' },
              { x: -12, z: 82, name: 'Pvt. Chen' },
              { x: 15, z: 80, name: 'Cpl. Diaz' },
              { x: 8, z: 85, name: 'Pvt. Foster' },
              { x: -5, z: 90, name: 'Sgt. Briggs' },
            ];
            for (const npc of standingPositions) {
              const g = new THREE.Group();
              const body = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.2, 0.4), npcMat);
              body.position.y = 0.6; g.add(body);
              const head = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.35), npcSkinMat);
              head.position.y = 1.4; g.add(head);
              const helmet = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.2, 0.4), npcMat);
              helmet.position.y = 1.6; g.add(helmet);
              g.position.set(npc.x, 0, npc.z);
              g.userData.npcName = npc.name;
              scene.add(g);
              game.c4BaseNPCs.push(g);
            }
            // Wounded NPCs (lying down) with medics
            const woundedPositions = [
              { x: -2, z: 105, name: 'Pvt. Hayes' },
              { x: 3, z: 108, name: 'Pvt. Kim' },
            ];
            for (const w of woundedPositions) {
              const g = new THREE.Group();
              const body = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.4, 1.2), npcMat);
              body.position.y = 0.2; g.add(body);
              const head = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.35), npcSkinMat);
              head.position.set(0, 0.3, 0.6); g.add(head);
              g.position.set(w.x, 0, w.z);
              g.userData.npcName = w.name;
              g.rotation.y = Math.random() * Math.PI * 2;
              scene.add(g);
              game.c4WoundedNPCs.push(g);
              // Medic nearby
              const mg = new THREE.Group();
              const mBody = new THREE.Mesh(new THREE.BoxGeometry(0.6, 1.2, 0.4), medicMat);
              mBody.position.y = 0.6; mg.add(mBody);
              const mHead = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.35), npcSkinMat);
              mHead.position.y = 1.4; mg.add(mHead);
              const mHelmet = new THREE.Mesh(new THREE.BoxGeometry(0.4, 0.2, 0.4), medicMat);
              mHelmet.position.y = 1.6; mg.add(mHelmet);
              // Red cross armband
              const armband = new THREE.Mesh(new THREE.BoxGeometry(0.62, 0.15, 0.1), new THREE.MeshBasicMaterial({ color: 0xcc3333 }));
              armband.position.set(0, 1.0, 0.22);
              mg.add(armband);
              mg.position.set(w.x + 1.5, 0, w.z);
              mg.userData.npcName = 'Medic';
              mg.userData.isMedic = true;
              scene.add(mg);
              game.c4MedicNPCs.push(mg);
            }
            game.c4PlayerCanMove = true;
            game.c4Phase = 'military_base';
            game.c4FadeDir = -1;
            game.c4CutsceneTime = 0;
          }
        } else if (game.c4Phase === 'military_base') {
          // Player can walk around the base and talk to NPCs
          camera.position.copy(game.playerPos);
          camera.rotation.set(game.pitch, game.yaw, 0);
          // Check proximity to NPCs for interaction
          if (game.c4InteractCooldown > 0) game.c4InteractCooldown -= dt;
          game.c4InteractTarget = null;
          const allNPCs = [...game.c4BaseNPCs, ...game.c4WoundedNPCs, ...game.c4MedicNPCs];
          for (const npc of allNPCs) {
            const dist = game.playerPos.distanceTo(npc.position);
            if (dist < 3) {
              game.c4InteractTarget = npc.userData.npcName;
              break;
            }
          }
        } else if (game.c4Phase === 'scout_area') {
          // PATH B: Scout the area -> find stranger -> Briggs shoots them
          camera.position.copy(game.playerPos);
          camera.rotation.set(game.pitch, game.yaw, 0);
          // Show stranger when player gets close to scouting area
          if (game.c4StrangerRef && !game.c4StrangerRef.visible) {
            const distToStranger = game.playerPos.distanceTo(new THREE.Vector3(20, 0, 3));
            if (distToStranger < 15) {
              game.c4StrangerRef.visible = true;
            }
          }
          // When player gets very close to stranger
          if (game.c4StrangerRef) {
            const distToStranger = game.playerPos.distanceTo(game.c4StrangerRef.position);
            if (distToStranger < 5 && !game.c4StrangerShot) {
              // Briggs shoots the stranger
              game.c4StrangerShot = true;
              game.c4Phase = 'briggs_shoots_stranger';
              game.c4CutsceneTime = 0;
              game.c4PlayerCanMove = false;
            }
          }
        } else if (game.c4Phase === 'briggs_shoots_stranger') {
          // Cutscene: Briggs shoots stranger
          if (game.c4BriggsRef) {
            game.c4BriggsRef.visible = true;
            // Move Briggs between player and stranger
            const briggsTarget = new THREE.Vector3(8, 0, 3);
            game.c4BriggsRef.position.lerp(briggsTarget, dt * 3);
          }
          if (t > 1.5) {
            // 'Gunshot' effect - stranger falls
            if (game.c4StrangerRef) {
              game.c4StrangerRef.rotation.x = Math.PI / 2 * Math.min((t - 1.5) / 0.5, 1);
              game.c4StrangerRef.position.y = -0.5 * Math.min((t - 1.5) / 0.5, 1);
            }
            // Stranger's last words
            game.c4DialogueSpeaker = '?';
            game.c4DialogueText = 'Sergeant...?';
            game.c4DialogueCharIdx = 0;
            game.c4DialogueLastCharTime = performance.now();
          }
          if (t > 5) {
        ...
        } else if (game.c4Phase === 'ending_fade') {
          // Fade to black and end mission
          if (game.c4FadeAlpha >= 1 && !game.c4WaitingForEnd) {
            game.c4WaitingForEnd = true;
            game.c4EndTime = performance.now();
          }
          if (game.c4WaitingForEnd && performance.now() - game.c4EndTime > 2000) {
            // End the mission
            onMatchEnd([{
              id: 'player',
              name: playerName,
  ...
  return (
    <div id="game-stage-container" ref={containerRef} className="relative w-full h-full bg-slate-950 overflow-hidden">
      ...
      {/* Campaign 4 dialogue overlay */}
      {config.isCampaign && config.mapId === 'campaign4' && (
        <div className='absolute bottom-12 left-8 z-40 pointer-events-none'>
          <div className='bg-black/80 border border-red-500/40 backdrop-blur-sm px-5 py-3 rounded-xl max-w-lg'>
            <p ref={c4DialogueSpeakerRef} className='text-red-400 font-mono text-sm font-bold mb-1 min-h-[20px]'></p>
            <p ref={c4DialogueTextRef} className='text-red-100/90 font-mono text-base min-h-[24px]'></p>
          </div>
        </div>
      )}

      {/* Campaign 4 fade overlay */}
      {config.isCampaign && config.mapId === 'campaign4' && (
        <div
          ref={c4FadeRef}
          className='absolute inset-0 z-50 pointer-events-none bg-black'
      ...
      {config.isCampaign && config.mapId === 'campaign4' && (
        <div className='absolute bottom-32 left-1/2 -translate-x-1/2 z-40 flex gap-6'>
          ...
          >
            ...
            <span className='choice-label text-slate-300 font-mono text-sm'>Keep driving</span>
          </button>
          <button
            ref={c4Choice2Ref}
          ...
          >
            ...
            <span className='choice-label text-slate-300 font-mono text-sm'>Scout the area</span>
          </button>
        </div>
      )}

      {/* Campaign 4 interaction prompt */}
      {config.isCampaign && config.mapId === 'campaign4' && gameRef.current.c4Phase === 'military_base' && gameRef.current.c4InteractTarget && (
        <div className='absolute bottom-24 left-1/2 -translate-x-1/2 z-40 pointer-events-none'>
          <div className='bg-black/60 border border-slate-500/40 backdrop-blur-sm px-4 py-2 rounded-lg'>
            <p className='text-slate-300 font-mono text-sm text-center'>
              Press <span className='text-emerald-400 font-bold'>F</span> to talk to <span className='text-yellow-400'>{gameRef.current.c4InteractTarget}</span>
            </p>
          </div>
        </div>
      )}
      ...
      {!isLocked && !isOverlayDismissed && !gameRef.current.playerIsDead && !(config.isCampaign && config.mapId === 'campaign3') && !(config.isCampaign && (config.mapId === 'campaign4' && !gameRef.current.c4PlayerCanMove)) && (
        <div
