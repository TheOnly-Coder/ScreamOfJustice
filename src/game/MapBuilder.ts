...
export function buildMap(scene: THREE.Scene, mapId: 'shipment' | 'rust' | 'dust2' | 'nuketown' | 'teams_combo' | 'tutorial' | 'campaign2' | 'campaign3' | 'campaign4'): MapData {
  ...
  if (mapId === 'nuketown') {
  ...
  } else if (mapId === 'campaign4') {
    // CAMPAIGN 4: 'The Signal' - Desert road with scouting area and military base
    // Tense desert atmosphere
    ambientColor = '#556677';
    ...
    // === GROUND ===
    const groundGeo = new THREE.PlaneGeometry(400, 600);
    const desertGroundMat = new THREE.MeshStandardMaterial({ color: 0x6b5d4f, roughness: 0.95, flatShading: true });
    const ground = new THREE.Mesh(groundGeo, desertGroundMat);
    ground.rotation.x = -Math.PI / 2;
    ground.position.set(0, 0, 0);
    ground.receiveShadow = true;
    scene.add(ground);
    ...
    // === ROAD ===
    const roadGeo = new THREE.PlaneGeometry(7, 400);
    const roadMat = new THREE.MeshStandardMaterial({ color: 0x44403a, roughness: 0.92, flatShading: true });
    const road = new THREE.Mesh(roadGeo, roadMat);
    road.rotation.x = -Math.PI / 2;
    ...
    const lineMat = new THREE.MeshStandardMaterial({ color: 0x888866, roughness: 0.8 });
    const centerLine = new THREE.Mesh(lineGeo, lineMat);
    centerLine.rotation.x = -Math.PI / 2;
    centerLine.position.set(0, 0.04, 50);
    scene.add(centerLine);

    // === SKY (overcast/tense) ===
    const skyGeo = new THREE.SphereGeometry(250, 32, 16, 0, Math.PI * 2, 0, Math.PI / 2);
    const skyMat = new THREE.MeshBasicMaterial({ color: 0x445566, side: THREE.BackSide });
    const sky = new THREE.Mesh(skyGeo, skyMat);
    sky.position.set(0, 0, 50);
    scene.add(sky);

    // === SCOUTING AREA (right side, z = -20 to 20) ===
    const concreteMat = new THREE.MeshStandardMaterial({ color: 0x777770, roughness: 0.85, flatShading: true });
    const sandbagMat = new THREE.MeshStandardMaterial({ color: 0x8b7d6b, roughness: 0.9, flatShading: true });

    // Concrete barriers
    const barrierPositions = [
      [15, 0.5, -15], [18, 0.5, -8], [20, 0.5, 0], [17, 0.5, 8], [14, 0.5, 15],
    ];
    for (const [bx, by, bz] of barrierPositions) {
      const barrier = new THREE.Mesh(new THREE.BoxGeometry(3, 1, 1), concreteMat);
    ...
    // Sandbag walls
    const sandbagWall1 = new THREE.Mesh(new THREE.BoxGeometry(1, 1.2, 6), sandbagMat);
    sandbagWall1.position.set(22, 0.6, -5);
    sandbagWall1.castShadow = true;
    scene.add(sandbagWall1);

    const sandbagWall2 = new THREE.Mesh(new THREE.BoxGeometry(1, 1.2, 6), sandbagMat);
    sandbagWall2.position.set(22, 0.6, 10);
    sandbagWall2.castShadow = true;
    scene.add(sandbagWall2);

    // Rocks in scouting area
    const rockMat = new THREE.MeshStandardMaterial({ color: 0x6a6055, roughness: 0.9, flatShading: true });
    const rockPositions = [
      [12, 0.5, -10, 1.5, 1, 1.2],
      [25, 0.4, 5, 1, 0.8, 0.8],
      [16, 0.6, 18, 1.8, 1.2, 1],
      [24, 0.3, -15, 0.8, 0.6, 0.7],
    ];
    for (const [rx, ry, rz, rw, rh, rd] of rockPositions) {
    ...
    // Dead trees
    const treeTrunkMat = new THREE.MeshStandardMaterial({ color: 0x4a3a2a, roughness: 0.9, flatShading: true });
    const treePositions = [10, 13, 19, 26];
    for (const tx of treePositions) {
      const tz = -20 + Math.random() * 40;
      const trunk = new THREE.Mesh(new THREE.BoxGeometry(0.3, 4, 0.3), treeTrunkMat);
      trunk.position.set(tx, 2, tz);
      ...
      // A few bare branches
      for (let b = 0; b < 3; b++) {
        const branch = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.15, 1.5), treeTrunkMat);
        branch.position.set(tx + (Math.random()-0.5)*0.3, 3 + b*0.5, tz + (Math.random()-0.5)*0.5);
        branch.rotation.z = (Math.random()-0.5) * 1.2;
        scene.add(branch);
      }
    }

    // === STRANGER POSITION (where the stranger stands in scouting area) ===
    // No model needed, the GameCanvas will create an NPC group here
    // The stranger stands at approximately (20, 0, 3) in the scouting area

    // === MILITARY BASE AREA (z = 60 to 120) ===
    const baseWallMat = new THREE.MeshStandardMaterial({ color: 0x5a5a55, roughness: 0.85, flatShading: true });
    const baseFloorMat = new THREE.MeshStandardMaterial({ color: 0x555550, roughness: 0.9, flatShading: true });

    // Base perimeter walls
    const perimeterWalls = [
      // Front wall (z=60, with gap for entrance)
      { x: -8, z: 60, w: 16, h: 3, d: 0.5 },
      { x: 12, z: 60, w: 20, h: 3, d: 0.5 },
      // Back wall
      { x: 0, z: 120, w: 50, h: 3, d: 0.5 },
      // Left wall
      { x: -25, z: 90, w: 0.5, h: 3, d: 60 },
      // Right wall
      { x: 25, z: 90, w: 0.5, h: 3, d: 60 },
    ];
    for (const w of perimeterWalls) {
      const wall = new THREE.Mesh(new THREE.BoxGeometry(w.w, w.h, w.d), baseWallMat);
    ...
    // Base floor
    const baseFloor = new THREE.Mesh(new THREE.PlaneGeometry(50, 60), baseFloorMat);
    baseFloor.rotation.x = -Math.PI / 2;
    baseFloor.position.set(0, 0.03, 90);
    baseFloor.receiveShadow = true;
    scene.add(baseFloor);

    // Base buildings
    const baseBuildingMat = new THREE.MeshStandardMaterial({ color: 0x4a4a45, roughness: 0.85, flatShading: true });
    const baseBuildingDarkMat = new THREE.MeshStandardMaterial({ color: 0x3a3a35, roughness: 0.9, flatShading: true });

    // Barracks (left side)
    const barracks = new THREE.Mesh(new THREE.BoxGeometry(12, 4, 8), baseBuildingMat);
    barracks.position.set(-15, 2, 80);
    barracks.castShadow = true;
    barracks.receiveShadow = true;
    scene.add(barracks);

    // Command center (right side, larger)
    const commandCenter = new THREE.Mesh(new THREE.BoxGeometry(14, 5, 10), baseBuildingDarkMat);
    commandCenter.position.set(12, 2.5, 85);
    commandCenter.castShadow = true;
    commandCenter.receiveShadow = true;
    scene.add(commandCenter);

    // Medical tent (center-back)
    const medTentMat = new THREE.MeshStandardMaterial({ color: 0x6a6a55, roughness: 0.8, flatShading: true });
    const medTent = new THREE.Mesh(new THREE.BoxGeometry(10, 3.5, 8), medTentMat);
    medTent.position.set(0, 1.75, 105);
    medTent.castShadow = true;
    scene.add(medTent);

    // Red cross on med tent
    const crossMat = new THREE.MeshBasicMaterial({ color: 0xcc3333 });
    const crossH = new THREE.Mesh(new THREE.BoxGeometry(2, 0.3, 0.1), crossMat);
    crossH.position.set(0, 2.5, 100.96);
    scene.add(crossH);
    const crossV = new THREE.Mesh(new THREE.BoxGeometry(0.3, 2, 0.1), crossMat);
    crossV.position.set(0, 2.5, 100.96);
    scene.add(crossV);

    // Supply crates
    const crateMat = new THREE.MeshStandardMaterial({ color: 0x6b5d3f, roughness: 0.85, flatShading: true });
    const cratePositions = [[-8, 0.6, 75], [-6, 0.6, 76], [18, 0.6, 78], [5, 0.6, 100], [-3, 0.6, 102]];
    ...
    const wheelGeo = new THREE.CylinderGeometry(0.5, 0.5, 0.3, 8);
    const wheelMat = new THREE.MeshStandardMaterial({ color: 0x222222, roughness: 0.9 });
    for (const [wx, wz] of [[-1.5, 2], [1.5, 2], [-1.5, -2.5], [1.5, -2.5]] as [number,number][]) {
      const wheel = new THREE.Mesh(wheelGeo, wheelMat);
      wheel.rotation.z = Math.PI / 2;
      wheel.position.set(wx, 0.5, wz);
      truckGroup.add(wheel);
    ...
    // === LIGHTING ===
    const ambLight = new THREE.AmbientLight(0x556677, 0.5);
    scene.add(ambLight);
    ...
    // Spawn point (near truck, player starts here after cutscene)
    spawnPoints.push(
